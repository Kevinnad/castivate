...
@SuppressLint("NewApi")
public class MediaControllerCompressor {
    ...
    private static volatile MediaControllerCompressor Instance = null;
    private boolean videoConvertFirstWrite = true;
    String compressfile;

    public static MediaControllerCompressor getInstance() {
        MediaControllerCompressor localInstance = Instance;
        if (localInstance == null) {
            synchronized (MediaControllerCompressor.class) {
                localInstance = Instance;
                if (localInstance == null) {
                    Instance = localInstance = new MediaControllerCompressor();
                }
            }
        }
    ...
    public static class VideoConvertRunnable implements Runnable {
        ...
        @Override
        public void run() {
            MediaControllerCompressor.getInstance().convertVideo(videoPath);
        }
    }
    ...
    public String convertVideo(final String path) {
        ...
        File cacheFile = new File(
                Environment.getExternalStorageDirectory()
                        + File.separator
                        + ConfigCompressor.VIDEO_COMPRESSOR_APPLICATION_DIR_NAME
                        + ConfigCompressor.VIDEO_COMPRESSOR_COMPRESSED_VIDEOS_DIR,
                "VIDEO_" + new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(new Date()) + ".mp4"
        );

        compressfile = cacheFile.getAbsolutePath();
