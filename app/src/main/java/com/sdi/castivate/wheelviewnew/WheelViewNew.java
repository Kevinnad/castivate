...
public class WheelViewNew extends ScrollView {
    public static final String TAG = WheelViewNew.class.getSimpleName();
    ...
    public WheelViewNew(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public WheelViewNew(Context context, AttributeSet attrs, int defStyle) {
...
//                    Log.d(TAG, "remainder: " + remainder + ", divided: " + divided);
                    if (remainder == 0) {
                        ...
                        if (remainder > itemHeight / 2) {
                            WheelViewNew.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    WheelViewNew.this.smoothScrollTo(0, initialY - remainder + itemHeight);
                            ...
                            WheelViewNew.this.post(new Runnable() {
                                @Override
                                public void run() {
                                    WheelViewNew.this.smoothScrollTo(0, initialY - remainder);
                                    selectedIndex = divided + offset;
                                    onSeletedCallBack();
                                }
                            });
                    ...
                    initialY = getScrollY();
                    WheelViewNew.this.postDelayed(scrollerTask, newCheck);
                }
            }
        };
...
//            position = divided + offset + 1;
        ...
        for (int i = 0; i < childSize; i++) {
            ...
            if (position == i) {
                //  itemView.setTextColor(Color.parseColor("#737272"));
                itemView.setTextColor(Color.BLACK);
            } else {
//                itemView.setTextColor(Color.parseColor("#F0F0F0"));

                itemView.setTextColor(Color.parseColor("#9b9898"));
            }
        }
    }
    ...
    public void setSeletion(int position) {
        ...
        this.post(new Runnable() {
            @Override
            public void run() {
                WheelViewNew.this.smoothScrollTo(0, p * itemHeight);
            }
        });
